...
// $sql2="SELECT * FROM registration WHERE room = "$room"";

$check="SELECT * FROM registration WHERE roomno= '$roomno'";
$result=$mysqli->query($check) or die(mysqli_error($check));
if (mysqli_num_rows($result)>0){
	echo"<script>alert('Room already exists ');</script>";
}else{
	$query="insert into  registration(roomno,feespm,stayfrom,duration,course,regno,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,pmntAddress,pmntCity) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	$stmt = $mysqli->prepare($query);
	$rc=$stmt->bind_param('iisisissssisississss',$roomno,$feespm,$stayfrom,$duration,$course,$regno,$fname,$mname,$lname,$gender,$contactno,$emailid,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$paddress,$pcity);
	$stmt->execute();
...
$uid=$_SESSION['login'];
				...
				if($rs)
				{ ?>
				<!-- <h3 style="color: red" align="left"></h3> -->
				<?php }
				else{
							echo "";
							}			
...
<div class="col-sm-8">
	<select name="Room Type" id="Room Type" class="form-control"> <option value="Select Room Type"></option>
